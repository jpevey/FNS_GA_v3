...
def pre_plotting(all_data, data):
    ...
    plt.scatter(x_vals, y_vals, color='r', s=10, label="Generation 50 Parents")
    ...
    plt.tight_layout()
    plt.savefig('pareto_rep_vs_totalflux_50gen.png')
    plt.close()
    #print(z_vals)
    ...
    ax1.set_title('K, Fast Flux, and Representativity for Gen 50 parents')
    plt.tight_layout()
    fig.savefig('k_ff_rep_gen50parents.png')
    plt.close()
    ...
    plt.title("Disk Counts for Gen. 50 Parents")
    plt.xlim(0,len(ind))
